...
/**
 * Initialize prefix list for alphabet-based filtering
 * Supports single letter (a-z) and double letter combinations (aa-zz)
 */
function initializePrefixList(useDoubleLetters = false) {
  if (!useDoubleLetters) {
    // Just use single letters a-z
    progressTracker.pending = [...ALPHABET];
  } else {
    // Use double-letter combinations (aa, ab, ac, ..., zz)
    progressTracker.pending = [];
    for (const first of ALPHABET) {
      for (const second of ALPHABET) {
        progressTracker.pending.push(first + second);
      }
    }
  }
  
  console.log(`Initialized ${progressTracker.pending.length} search prefixes`);
}
...
async function processAllPrefixes() {
  try {
    ...
    if (!trackerExists) {
      // Initialize with single-letter prefixes
      // Set to true for double-letter combinations (aa-zz)
      initializePrefixList(false);
